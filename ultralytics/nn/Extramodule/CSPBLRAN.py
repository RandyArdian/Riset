...
class CSPBLRAN(nn.Module):
    def __init__(self, in_channels, out_channels, n_blocks=6, expand=0.5, **block_kwargs):
        super().__init__()
        hidden = int(out_channels * expand)

        self.cv1 = nn.Conv2d(in_channels, hidden * 2, 1, 1, 0, bias=False)
        self.bn1 = nn.BatchNorm2d(hidden * 2)
        self.act = nn.SiLU(inplace=True)

        self.blocks = nn.Sequential(
            *[Block(dim=hidden, **block_kwargs) for _ in range(n_blocks)]
        )

        self.cv2 = nn.Conv2d(hidden * 2, out_channels, 1, 1, 0, bias=False)
        self.bn2 = nn.BatchNorm2d(out_channels)

    def forward(self, x):
        x = self.act(self.bn1(self.cv1(x)))
        x1, x2 = torch.chunk(x, 2, dim=1)  # First & Second Partition
